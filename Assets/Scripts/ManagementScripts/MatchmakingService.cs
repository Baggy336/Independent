...
public static class MatchmakingService
{
    ...
    // Query for open lobbies in the lobby manager
    ...
    // Create a lobby with data parameters, refresh with the data immediately
    ...
    // Lock a lobby for no other player to join
    ...
    // Every so often, update the information shown in the lobby
    ...
    public static async Task LeaveLobby()
    {
        ...
        if (_currentLobby != null)
            try
            {
                if (_currentLobby.HostId == Authenticator.PlayerId) await Lobbies.Instance.DeleteLobbyAsync(_currentLobby.Id);
                else await Lobbies.Instance.RemovePlayerAsync(_currentLobby.Id, Authenticator.PlayerId);
                _currentLobby = null;
            }
            catch (Exception e)
            {
